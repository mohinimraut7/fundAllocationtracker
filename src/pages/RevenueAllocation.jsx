...
//       // ✅ backend दोन्ही प्रकारे data देऊ शकतो:
//       // 1) res.json(data)
//       // 2) res.json({ success: true, data })
...
//         if (roleWiseData.length > 0) {
//           const latest = roleWiseData[0]; // ✅ because backend ideally sort -1 (latest first)
...
//       // ✅ file upload साठी FormData MUST
//       const formData = new FormData();
//       formData.append("totalRevenue", Number(totalRevenue));
//       formData.append("allocatedAmount", Number(allocation));
//       formData.append("date", todayDate);
//       formData.append("role", role);
//       formData.append("file", attachment); // ✅ multer field name = file
...
//       // ✅ IMPORTANT: backend expects field name = "attachment"
...
export default function RevenueAllocation() {
  ...
  // ✅ NEW: attachmentUrl store (open file)
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (roleWiseData.length > 0) {
          ...
          // ✅ NEW: set attachment url for click open
          setSavedAttachmentUrl(latest.attachmentUrl || null);
        } else {
          setSavedRevenue(null);
          setAllocatedAmount(null);
          setSavedDate(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ NEW: set url after save
      setSavedAttachmentUrl(result.data.attachmentUrl || null);
                ...
                <tr className="border-t hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 text-blue-600 font-medium">
                    {savedAttachmentUrl ? (
                      ...
                      >
                        {savedAttachmentName || "View Document"}
                      </a>
                    ) : (
                      "No Document"
                    )}
